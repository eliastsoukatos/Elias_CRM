...
def get_db_path():
    ...
    if os.environ.get("DB_PATH") and os.name == 'nt':  # Only use on Windows
        ...
        print(
            f"Using hardcoded database path from environment: {hardcoded_path}")
        ...
        try:
            ...
            if not os.path.exists(db_dir):
                try:
                    os.makedirs(db_dir, exist_ok=True)
                    print(
                        f"Created fallback database directory in user home: {db_dir}")
...
class ContactCampaignWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.campaign_list.setHorizontalHeaderLabels(
            ["ID", "Campaign Name", "Created Date", "Total Contacts"])
        ...
        clear_campaign_button.setStyleSheet(
            "background-color: #f44336; color: white;")
        actions_layout.addWidget(clear_campaign_button)

        # Store buttons for enabling/disabling
        self.action_buttons = [add_contacts_button, view_tags_button,
                               run_prospector_button, clear_campaign_button]
    ...
    def load_campaigns(self):
        ...
        try:
            ...
            for row, campaign in enumerate(campaigns):
                ...
                self.campaign_list.setItem(
                    row, 0, QTableWidgetItem(str(campaign_id)))
                self.campaign_list.setItem(row, 1, QTableWidgetItem(name))
                self.campaign_list.setItem(
                    row, 2, QTableWidgetItem(created_at))
                self.campaign_list.setItem(
                    row, 3, QTableWidgetItem(str(contact_count)))
            ...
            QMessageBox.critical(
                self, "Error", f"Error loading campaigns: {str(e)}")
    ...
    def update_campaign_stats(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Error updating campaign stats: {str(e)}")
    ...
    def add_contacts_to_campaign(self):
        ...
        self.search_and_add_contacts_dialog = SearchContactsDialog(
            self.selected_campaign_id, self.selected_campaign_name)
        self.search_and_add_contacts_dialog.contactsAdded.connect(
            self.update_campaign_stats)
        ...
        try:
            ...
            if not batch_tags:
                QMessageBox.information(
                    self, "No Batch Tags", "No batch tags found for this campaign.")
                return

            # Create a dialog to display the batch tags
            dialog = QDialog(self)
            dialog.setWindowTitle(
                f"Batch Tags for {self.selected_campaign_name}")
            ...
            QMessageBox.critical(
                self, "Error", f"Error viewing batch tags: {str(e)}")
    ...
    def run_contact_prospector(self):
        ...
        try:
            ...
            script_path = os.path.join(os.path.dirname(
                os.path.abspath(__file__)), 'run_contact_prospector.py')
            ...
            subprocess.Popen([sys.executable, script_path,
                             str(self.selected_campaign_id)])

        except Exception as e:
            QMessageBox.critical(
                self, "Error", f"Error launching contact prospector: {str(e)}")
    ...
    def clear_campaign_contacts(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Error clearing campaign contacts: {str(e)}")
...
class SearchContactsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # Header
        header_label = QLabel(
            f"Add Contacts to Campaign: {self.campaign_name}")
        ...
        # SQL Query input
        query_label = QLabel(
            "SQL Query (contacts from approved companies in this campaign):")
        ...
        query_help = QLabel(
            "NOTE: Query must return contact_id, Name, Last_Name, Role, Email, LinkedIn_URL, and company_name.")
        ...
        self.results_table.setHorizontalHeaderLabels(
            ["ID", "Name", "Role", "Email", "Company", "Select"])
        ...
        add_selected_button.setStyleSheet(
    ...
    def load_approved_companies(self):
        ...
        try:
            ...
            self.company_count_label.setText(
                f"Found {len(approved_companies)} approved companies in this campaign.")
            ...
            QMessageBox.critical(
                self, "Error", f"Error loading approved companies: {str(e)}")
    ...
    def load_contact_tags(self):
        ...
        try:
            ...
            if not tags:
                QMessageBox.information(
                    self, "No Tags", "No contact tags found for contacts in approved companies.")
                return

            # Add tags to combo box
            for tag, count in tags:
                if tag:  # Only add non-empty tags
                    self.tag_filter_combo.addItem(
                        f"{tag} ({count} contacts)", tag)

            QMessageBox.information(
                self, "Tags Loaded", f"Loaded {len(tags)} contact tags.")

        except Exception as e:
            QMessageBox.critical(
                self, "Error", f"Error loading contact tags: {str(e)}")
    ...
    def apply_tag_filter(self):
        ...
        if selected_tag == "all":
            QMessageBox.information(
                self, "No Filter Applied", "Using all contacts (no tag filter).")
            return
...
ORDER BY c.Name, c.Last_Name""".format(self.campaign_id)
        ...
        if from_index == -1 or where_index == -1:
            QMessageBox.warning(
                self, "Invalid Query", "Could not parse query. Please ensure it has FROM and WHERE clauses.")
            return
        ...
        if has_join:
            for i, line in enumerate(lines):
                if "contacts_cognism_urls.contact_tag" in line:
                    lines[i] = "    AND contacts_cognism_urls.contact_tag = '{}' -- Tag filter".format(
                        selected_tag)
                    break
        else:
            ...
            # Add WHERE condition
            where_clause = "    AND contacts_cognism_urls.contact_tag = '{}' -- Tag filter".format(
                selected_tag)
        ...
        QMessageBox.information(
            self, "Tag Filter Applied", f"Query updated to filter for tag: {selected_tag}")

    def run_sql_query(self):
        """Run custom SQL query to find contacts"""
        if not hasattr(self, 'approved_company_ids') or not self.approved_company_ids:
            QMessageBox.warning(
                self, "No Companies", "No approved companies found for this campaign.")
            return
        ...
        try:
            ...
            required_columns = ['contact_id', 'Name', 'Last_Name',
                                'Role', 'Email', 'LinkedIn_URL', 'company_name']
            missing_columns = [
            ...
            if not contacts:
                QMessageBox.information(
                    self, "No Results", "No contacts found matching your query.")
                return
            ...
            for row, contact in enumerate(contacts):
                ...
                self.results_table.setItem(
                    row, 0, QTableWidgetItem(str(contact_id)))
                ...
                self.results_table.setItem(
                    row, 4, QTableWidgetItem(company_name))
            ...
            # Show count
            QMessageBox.information(
                self, "Query Results", f"Found {len(contacts)} contacts matching your query.")

        except Exception as e:
            QMessageBox.critical(
                self, "SQL Error", f"Error executing query: {str(e)}")
    ...
    def add_selected_contacts(self):
        ...
        if not selected_contact_ids:
            QMessageBox.warning(self, "No Contacts Selected",
        ...
        try:
            ...
            for contact_id in selected_contact_ids:
                try:
                    ...
                    cursor.execute(
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                        """,
                        (contact_id, self.campaign_id, self.campaign_name,
                         batch_tag, batch_id, company_id, "")
                    )
                    added_count += 1
            ...
            QMessageBox.critical(
                self, "Error", f"Error adding contacts to campaign: {str(e)}")
