...
def load_previous_mappings(file_path=None):
    ...
    mappings_path = os.path.join(
        project_root, 'databases', 'header_mappings.json')
...
def create_new_column(table, column):
    ...
    try:
        ...
        conn.close()
        print(
            f"✅ Se ha creado la nueva columna '{column}' en la tabla '{table}'.")
    except Error as e:
...
def create_new_table(table_name):
    ...
    try:
        ...
        conn.close()
        print(
            f"✅ Successfully created new table '{table_name}' with company_id foreign key.")
        return True
    except Error as e:
...
def import_csv():
    ...
    try:
        ...
        file_path = input(
            "Please enter the path or name of the CSV file you want to import: ").strip()
        ...
        for path in possible_paths:
            ...
            print(
                f"{Colors.RED}❌ Invalid file. Please ensure the CSV file exists and has a '.csv' extension.{Colors.END}")
        ...
        if previous_mappings and set(previous_mappings.keys()) == set(csv_headers):
            ...
            use_prev = input(
                f"Do you want to apply the previous mapping to all columns? ({Colors.GREEN}yes{Colors.END}/{Colors.RED}no{Colors.END}): ").strip().lower()
            if use_prev == "yes":
                header_mapping = previous_mappings
                print(
                    f"{Colors.GREEN}✅ Applied previous mappings to all columns.{Colors.END}")
            else:
                ...
                for header in csv_headers:
                    if header in previous_mappings:
                        ...
                        print(
                            f"{Colors.YELLOW}Previous mapping:{Colors.END} '{Colors.BOLD}{previous_mappings[header]}{Colors.END}'")
                        use_mapping = input(
                            f"Keep this mapping? ({Colors.GREEN}yes{Colors.END}/{Colors.RED}no{Colors.END}): ").strip().lower()
                        if use_mapping == "yes":
                            header_mapping[header] = previous_mappings[header]
                            print(
                                f"{Colors.GREEN}✓ Kept previous mapping{Colors.END}")
                            continue
                    valid_mapping = False
                        ...
                        if user_mapping.lower() == "skip":
                            ...
                            if create_option == "1":
                                ...
                                for i, table in enumerate(sorted(db_columns.keys()), 1):
                                    print(
                                        f"  {Colors.CYAN}{i}.{Colors.END} {table}")

                                chosen_table = input(
                                    f"\n{Colors.BOLD}Enter the table name:{Colors.END} ").strip()
                                if chosen_table in db_columns:
                                    new_col = input(
                                        f"Enter the new column name for CSV header '{Colors.BOLD}{header}{Colors.END}' (default: use CSV header name): ").strip()
                                    if not new_col:
                                        new_col = header
                                    ...
                                    print(
                                        f"{Colors.GREEN}✅ Created new column '{new_col}' in table '{chosen_table}'{Colors.END}")
                                    valid_mapping = True
                                else:
                                    print(
                                        f"{Colors.RED}❌ Invalid table name. Please choose one of the listed tables.{Colors.END}")
                            elif create_option == "2":
                                # Create new table and column
                                new_table = input(
                                    f"{Colors.BOLD}Enter the name for the new table (use snake_case, e.g. company_certifications):{Colors.END} ").strip()
                                if not new_table:
                                    print(
                                        f"{Colors.RED}❌ Table name cannot be empty.{Colors.END}")
                                    continue

                                # Validate table name format
                                if not all(c.islower() or c.isdigit() or c == '_' for c in new_table):
                                    print(
                                        f"{Colors.YELLOW}⚠️ Table name should be in snake_case. Converting to lowercase with underscores.{Colors.END}")
                                    new_table = new_table.lower().replace(' ', '_')

                                # Create the new table
                                if create_new_table(new_table):
                                    # Add the new table to db_columns
                                    db_columns[new_table] = [
                                        "id", "company_id"]
                                    ...
                                    if not all(c.islower() or c.isdigit() or c == '_' for c in new_col):
                                        print(
                                            f"{Colors.YELLOW}⚠️ Column name should be in snake_case. Converting to lowercase with underscores.{Colors.END}")
                                    ...
                                    print(
                                        f"{Colors.GREEN}✅ Created new table '{new_table}' with column '{new_col}'{Colors.END}")
                                    valid_mapping = True
                                else:
                                    print(
                                        f"{Colors.RED}❌ Failed to create new table. Please try again.{Colors.END}")
                            else:
                                print(
                                    f"{Colors.RED}❌ Invalid option. Please enter 1 or 2.{Colors.END}")
                            ...
                            if any(user_mapping in cols for cols in db_columns.values()):
                                ...
                                for table, columns in db_columns.items():
                                    if user_mapping in columns:
                                        ...
                                        break
                                print(
                                    f"{Colors.GREEN}✅ Mapped to '{table_name}.{user_mapping}'{Colors.END}")
                                valid_mapping = True
                            else:
                                print(
                                    f"{Colors.RED}❌ Invalid mapping. Please enter a valid database column, or choose 'skip' or 'new'.{Colors.END}")
        else:
            # No hay mapeos previos completos o no coinciden, proceder mapeo manual para cada header
            for header in csv_headers:
                if header in previous_mappings:
                    use_mapping = input(
                        f"Previous mapping for '{header}' is '{previous_mappings[header]}'. Do you want to keep it? (yes/no): ").strip().lower()
                    if use_mapping == "yes":
                        header_mapping[header] = previous_mappings[header]
                        continue
                valid_mapping = False
                while not valid_mapping:
                    user_mapping = input(
                        f"Enter your choice for '{header}': ").strip()
                    if user_mapping.lower() == "skip":
                        header_mapping[header] = None
                        valid_mapping = True
                    elif user_mapping.lower() == "new":
                        create_option = input(
                            "Do you want to create: (1) A new column in existing table, or (2) A new table and column? Enter 1 or 2: ").strip()

                        if create_option == "1":
                            # Create new column in existing table
                            chosen_table = input(
                                "Enter the table name where you want to create the new column: ").strip()
                            if chosen_table in db_columns:
                                new_col = input(
                                    f"Enter the new column name for CSV header '{header}' (default: use CSV header name): ").strip()
                                if not new_col:
                                    new_col = header
                                create_new_column(chosen_table, new_col)
                                db_columns[chosen_table].append(new_col)
                                ...
                                print(
                                    "❌ Invalid table name. Please choose one of the listed tables.")
                        elif create_option == "2":
                            # Create new table and column
                            new_table = input(
                                "Enter the name for the new table (use snake_case, e.g. company_certifications): ").strip()
                            if not new_table:
                            ...
                            if not all(c.islower() or c.isdigit() or c == '_' for c in new_table):
                                print(
                                    "❌ Table name should be in snake_case (lowercase with underscores).")
                                continue
                            ...
                            if create_new_table(new_table):
                                ...
                                if not all(c.islower() or c.isdigit() or c == '_' for c in new_col):
                                    print(
                                        "⚠️ Column name should be in snake_case. Converting to lowercase with underscores.")
                                    new_col = new_col.lower().replace(' ', '_')
                                ...
                                print(
                                    "❌ Failed to create new table. Please try again.")
                        else:
                        ...
                        if any(user_mapping in cols for cols in db_columns.values()):
                            ...
                            print(
                                "❌ Invalid mapping. Please enter a valid database column, or choose 'skip' or 'new'.")

        # Guardar los mapeos actualizados para futuras importaciones
        save_mappings(header_mapping)
        print(
            f"{Colors.GREEN}✅ Header mappings have been saved for future imports.{Colors.END}")
        ...
        while not valid_tag:
            batch_tag = input(
                f"{Colors.BOLD}Please enter a unique batch tag for this data load:{Colors.END} ").strip()
            ...
            if batch_tag and all(c.isalnum() or c in " -_" for c in batch_tag):
                ...
                print(
                    f"{Colors.RED}❌ Invalid batch tag. Please avoid special characters.{Colors.END}")

        # 9. Generar automáticamente un batch_id único
        batch_id = str(uuid.uuid4())
        print(
            f"{Colors.GREEN}🆔 Generated batch ID:{Colors.END} {Colors.BOLD}{batch_id}{Colors.END}")
        ...
        columns_to_drop = [header for header,
                           mapping in header_mapping.items() if mapping is None]
        if columns_to_drop:
        ...
        if website_columns:
            print(
                f"{Colors.CYAN}🌐 Standardizing website URLs to format: https://domain.com{Colors.END}")
            for col in website_columns:
                ...
                dataframe[col] = dataframe[col].apply(
                    lambda url: clean_url(url) if pd.notna(url) and url else "")
        ...
        valid_mapping = {header: mapping for header,
                         mapping in header_mapping.items() if mapping is not None}

        # Check for duplicate target column names
        target_columns = list(valid_mapping.values())
        duplicate_targets = [col for col in set(
            target_columns) if target_columns.count(col) > 1]
        ...
        if len(dataframe.columns) != len(set(dataframe.columns)):
            ...
            duplicates = {col: indices for col,
                          indices in seen.items() if len(indices) > 1}

            if duplicates:
                print(
                    "⚠️ Found duplicate columns after renaming. Fixing by adding suffixes:")
                ...
                for col, indices in duplicates.items():
                    ...
                    for i, idx in enumerate(indices[1:], 1):
                        ...
                        print(
                            f"  - Renaming duplicate column at position {idx} from '{col}' to '{new_name}'")
        ...
        print(
            f"{Colors.GREEN}✅ Data successfully mapped and converted to JSON format.{Colors.END}")
        ...
        preprocessor(data_json, batch_id, batch_tag,
                     extra_context=extra_context)
        print(
            f"{Colors.GREEN}🚀 Data preprocessing completed with batch ID: {batch_id}{Colors.END}")
