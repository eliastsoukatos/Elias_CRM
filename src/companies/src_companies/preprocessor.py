                    ...
                    try:
                        ...
                        if appdata:
                            app_db_path = os.path.join(
                                appdata, 'Elias_CRM', 'databases', 'database.db')
                            print(
                                f"🔍 Trying APPDATA database path: {app_db_path}")
    ...
    if '[' in address_str:
        ...
        for addr in addr_list:
            ...
            if len(parts) >= 3:
                ...
                if len(parts) >= 5:
                    zip_code = parts[4]
            office_type = "headquarters" if hq_location and hq_location.strip(
            ) and hq_location.strip() in addr else "sucursal"
        ...
        if len(parts) >= 3:
            ...
            if len(parts) >= 5:
                zip_code = parts[4]
        office_type = "headquarters" if hq_location and hq_location.strip(
        ) and hq_location.strip() in address_str else "sucursal"
        # Si todos los campos clave están vacíos, se omite el registro
    ...
    try:
        ...
        for record in data:
            ...
            if isinstance(record, dict) and "summary" in record and clutch_map_all:
                ...
                # Ignoramos direcciones de Clutch
                mapped["company_locations"] = []
                company_name = mapped.get("companies", {}).get(
                    "name", "").strip()  # conserva mayúsculas
                ...
                if record.get("phone_number"):
                    ...
                    if ',' in phone_number:
                        ...
                        phone_numbers = [
                            p.strip() for p in phone_number.split(',') if p.strip()]
                        for phone in phone_numbers:
                            mapped["company_phones"].append(
                                {"phone_number": phone})
                    else:
                        mapped["company_phones"].append(
                ...
                overall_rating = standardize_number(
                    record.get("overall_rating", None))
                review_count = standardize_number(
                    record.get("review_count", None))
                ...
                mapped["company_locations"] = parse_addresses(
                    record.get("locations", ""), record.get("hq_location", ""))
            ...
            if not company_name or not website:
                print(
                    f"⚠️ Skipped company due to missing mandatory fields: {company_name}")
                continue
            ...
            if existing_company_id:
                ...
                print(
                    f"🔄 Existing company found. Updating fields for: {company_name}")
            else:
                print(
                    f"✨ New company detected. Preparing data for insertion: {company_name}")
                ...
                for table, mappings in table_mappings.items():
                    ...
                    if table not in ["companies", "batch_tag", "batch_id", "source"]:
                        ...
                        if is_new_table:
                            ...
                            print(
                                f"  ➕ Processing custom table {table} with {len(mappings)} fields")
                            ...
                            windows_path = "/Users/anthonyhurtado/Jobs/personal/others/Elias_CRM/databases/database.db"
                            print(
                                f"🔍 Trying hardcoded Windows database path for schema lookup: {windows_path}")
                            ...
                            try:
                                ...
                                table_columns = [column[1]
                                                 for column in cursor.fetchall()]
                                cursor.close()
                                conn.close()
                                print(
                                    f"✅ Successfully retrieved schema from hardcoded path")
                            except Exception as e:
                                print(
                                    f"⚠️ Hardcoded path failed for schema lookup: {e}, trying alternatives")
                                ...
                                if not project_root:
                                    ...
                                    companies_dir = os.path.dirname(
                                        src_companies_dir)       # src dir
                                    project_root = os.path.dirname(
                                        companies_dir)            # project root

                                try:
                                    db_path = os.path.join(
                                        project_root, 'databases', 'database.db')
                                    print(
                                        f"🔍 Trying project root path for schema lookup: {db_path}")
                                    conn = sqlite3.connect(db_path)
                                    cursor = conn.cursor()
                                    cursor.execute(
                                    ...
                                    conn.close()
                                    print(
                                        f"✅ Successfully retrieved schema from project root path")
                                except Exception as e2:
                                    print(
                                        f"⚠️ Project root path failed for schema lookup: {e2}, trying home directory")

                                    # Try user's home directory
                                    try:
                                        user_home = os.path.expanduser("~")
                                        alt_db_path = os.path.join(
                                            user_home, 'databases', 'database.db')
                                        print(
                                            f"🔍 Trying home directory for schema lookup: {alt_db_path}")
                                        conn = sqlite3.connect(alt_db_path)
                                        cursor = conn.cursor()
                                        cursor.execute(
                                            f"PRAGMA table_info({table});")
                                        ...
                                        conn.close()
                                        print(
                                            f"✅ Successfully retrieved schema from home directory path")
                                    except Exception as e3:
                                        print(
                                            f"⚠️ Home directory path failed for schema lookup: {e3}")
                                        # If all attempts fail, provide a default set of columns
                                        table_columns = [
                                            "id", "company_id", "value"]
                                        print(
                                            f"⚠️ Using default columns: {table_columns}")

                            # Find out how many fields this table has (excluding id and company_id)
                            data_fields = [
                                col for col in table_columns if col not in ["id", "company_id"]]
                            ...
                            if len(data_fields) == 1:
                                ...
                                for csv_header, db_column in mappings:
                                    if csv_header in original_data and original_data[csv_header]:
                                        ...
                                        print(
                                            f"  ➕ Added new row in {table} with {field_name}='{value}' from CSV column '{csv_header}'")
                            else:
                                ...
                                for csv_header, db_column in mappings:
                                    if csv_header in original_data:
                                        ...
                                        print(
                                            f"  ➕ Added value '{value}' to {table}.{db_column} from CSV column '{csv_header}'")
                        else:
            ...
            db_writer(mapped)
            print(
                f"🚀 Data written to the database for company: {company_name}")
