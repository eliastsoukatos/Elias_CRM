...
def connect_db():
    ...
    # DEBUG OUTPUT
    print(
        f"[DEBUG] DB_WRITER Connect_db: Current working directory: {os.getcwd()}")
...
def db_writer(data):
    ...
    try:
        ...
        if verification_records:
            insert_related_table(cursor, "company_verifications",
                                 company_id, verification_records, ["verification_id"])
        ...
        if rating_records:
            insert_related_table(cursor, "company_ratings",
                                 company_id, rating_records, ["rating_id"])
                ...
                try:
                    ...
                    if cursor.fetchone():
                        ...
                        for record in table_data:
                            ...
                            print(
                                f"✅ Inserted record into custom table {table_name}")
                    else:
                        print(
                            f"⚠️ Custom table {table_name} not found in database. Skipping.")
                except Exception as e:
                    print(
                        f"🚨 Error inserting into custom table {table_name}: {e}")
        ...
        cursor.execute(
            "SELECT log_id FROM import_logs WHERE batch_id = ?", (data["batch_id"],))
